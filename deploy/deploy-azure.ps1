...
# Esegue un comando az catturando stdout+stderr; su errore stampa i dettagli e fa fatal.
# Non riesegue mai il comando (evita side-effect doppi).
function Invoke-Az {
    param([string[]]$Arguments)
    $result = az @Arguments 2>&1
    if ($LASTEXITCODE -ne 0) {
        $detail = ($result | Select-Object -Last 5) -join "`n"
        Fatal "az $($Arguments[0..2] -join ' ') ... fallito (exit $LASTEXITCODE):`n$detail"
    }
    return $result
}
...
$emptyKeys          = @()   # inizializzato qui per essere sempre disponibile nel summary
...
# Estensioni CLI necessarie
Info "Verifico estensioni Azure CLI (application-insights, authV2)..."
az extension add --name application-insights --only-show-errors 2>$null | Out-Null
az extension add --name authV2              --only-show-errors 2>$null | Out-Null
Ok "Estensioni CLI pronte"

# ─── 1. Login + subscription ────────────────────────────────────────────────
Section "1. Azure CLI login"

$account = az account show --output json 2>$null | ConvertFrom-Json
if (-not $account -or $account.tenantId -notlike $TenantId) {
...
if ($stExists -ieq $storageAccountName) {
...
if (-not $storageConnStr) { Fatal "Impossibile recuperare la connection string dello storage account." }

$queueExists = az storage queue exists --name $queueName --connection-string $storageConnStr --query exists --output tsv 2>$null
if ($queueExists -ieq "true") {
    Warn "Queue '$queueName' già esistente – skip."
} else {
    Info "Creo queue '$queueName'..."
    az storage queue create --name $queueName --connection-string $storageConnStr --output none 2>$null | Out-Null
...
if ($funcExists -ieq $functionAppName) {
...
Invoke-Az (@("functionapp","config","appsettings","set",
    "--name",$functionAppName,
    "--resource-group",$ResourceGroupName,
    "--settings") + $settingsArgs + @("--output","none")) | Out-Null
Ok "App Settings configurati"

$emptyKeys = $settings.GetEnumerator() |
...
if ($SpSiteUrl) {
    ...
    $spOrigin = "$($spUri.Scheme)://$($spUri.Host)"

    # Recupera le origini già configurate
    $existingOrigins = az functionapp cors show `
        --name $functionAppName `
        --resource-group $ResourceGroupName `
        --query "cors.allowedOrigins" --output tsv 2>$null

    if ($existingOrigins -icontains $spOrigin) {
        Warn "Origine CORS '$spOrigin' già presente – skip."
    } else {
        Info "Aggiungo origine CORS: $spOrigin"
...
if ($SkipDeploy) {
    ...
    # Pulisce la publish dir per evitare file stale tra deploy successivi
    if (Test-Path $publishDir) {
        Info "Pulizia publish dir precedente..."
        Remove-Item -Recurse -Force $publishDir
    }

    Info "Eseguo 'dotnet publish' in Release dal progetto be/..."
    # Usa try/finally per garantire il ripristino della directory anche in caso di errore
    Push-Location $beDir
    try {
...
if ([string]::IsNullOrWhiteSpace($AppRegClientId)) {
    ...
    Info "Configuro EasyAuth v2 (Entra ID) sulla Function App..."
    # 1. Imposta il provider Microsoft (v2 API).
    #    NOTA: --tenant-id e --issuer sono mutualmente esclusivi; --tenant-id genera
    #    automaticamente l'issuer corretto (login.microsoftonline.com/{tenantId}/v2.0).
    az webapp auth microsoft update `
        --name $functionAppName `
        --resource-group $ResourceGroupName `
        --client-id $AppRegClientId `
        ...
        --yes `
        --output none 2>$null | Out-Null
    # 2. Abilita EasyAuth v2 con AllowAnonymous: le richieste senza token passano al codice;
    #    le richieste con Bearer token valido ricevono l'iniezione di X-MS-CLIENT-PRINCIPAL-ID.
    #    NON usare --action (parametro v1) qui: sovrascriverebbre la config v2 appena impostata.
    az webapp auth update `
        --name $functionAppName `
        --resource-group $ResourceGroupName `
        --enabled true `
        --unauthenticated-client-action AllowAnonymous `
        --output none 2>$null | Out-Null
    Ok "EasyAuth v2 abilitato – provider: Microsoft / Entra ID"
    Ok "Token validi → X-MS-CLIENT-PRINCIPAL-ID iniettato. Richieste senza token → passano al codice."
}

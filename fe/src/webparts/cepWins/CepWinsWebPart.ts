...
import CepWins from './components/CepWins';
import { ICepWinsProps } from './components/ICepWinsProps';
import { CepApiClient } from '../../services/CepApiClient';

// Tenant Properties keys – same as all other CEP web parts
const TENANT_KEY_BASE_URL  = 'CEP_FunctionAppBaseUrl';
const TENANT_KEY_CLIENT_ID = 'CEP_FunctionAppClientId';

export type ICepWinsWebPartProps = Record<string, never>;

export default class CepWinsWebPart extends BaseClientSideWebPart<ICepWinsWebPartProps> {
  ...
  private async _loadTenantProperties(): Promise<void> {
    ...
    try {
      ...
      if (!this._functionAppBaseUrl || !this._functionAppClientId) {
        console.warn('[CepWins] Tenant Properties not found. Run deploy/set-tenant-properties.ps1.');
      }
    } catch (e) {
      console.error('[CepWins] Error retrieving Tenant Properties:', e);
    }
  }
  ...
  private async _initApiClient(): Promise<void> {
    ...
    try {
      ...
      console.error('[CepWins] Failed to initialise AAD HTTP client:', e);
    }
  }

  public render(): void {
    const element: React.ReactElement<ICepWinsProps> = React.createElement(
      CepWins,
      {
  ...
  protected getPropertyPaneConfiguration(): IPropertyPaneConfiguration {
    return {
      pages: [
        {
          header: { description: 'CEP Copilot Wins — no configuration required. Settings are read from SharePoint Tenant Properties.' },
          groups: []
        }
      ]
    };

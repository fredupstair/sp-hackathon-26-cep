...
import CepWelcome from './components/CepWelcome';
import { ICepWelcomeProps } from './components/ICepWelcomeProps';
...
export interface ICepWelcomeWebPartProps {
...
export default class CepWelcomeWebPart extends BaseClientSideWebPart<ICepWelcomeWebPartProps> {
  ...
  protected async onInit(): Promise<void> {
    ...
    try {
      ...
      console.warn('[CepWelcome] MSGraphClientV3 not available:', e);
    }
  }
  ...
  private async _loadTenantProperties(): Promise<void> {
    ...
    try {
      ...
      if (!this._functionAppBaseUrl || !this._functionAppClientId) {
        console.warn(
          '[CepWelcome] Tenant Properties not found. ' +
          'Run deploy/set-tenant-properties.ps1 to set them.'
        );
      }
    } catch (e) {
      console.error('[CepWelcome] Error retrieving Tenant Properties:', e);
    }
  }
  ...
  private async _initApiClient(): Promise<void> {
    ...
    try {
      ...
      console.error('[CepWelcome] Failed to initialise AAD HTTP client:', e);
    }
  }

  public render(): void {
    const element: React.ReactElement<ICepWelcomeProps> = React.createElement(
      CepWelcome,
      {
  ...
  protected getPropertyPaneConfiguration(): IPropertyPaneConfiguration {
    return {
      pages: [
        {
          header: { description: 'Copilot Engagement Program — welcome experience settings.' },
          groups: [
            {

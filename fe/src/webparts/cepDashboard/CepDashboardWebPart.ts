...
// Tenant Properties keys – same as CepOptinWebPart
...
export default class CepDashboardWebPart extends BaseClientSideWebPart<ICepDashboardWebPartProps> {
  ...
  private async _loadTenantProperties(): Promise<void> {
    ...
    try {
      ...
      if (!this._functionAppBaseUrl || !this._functionAppClientId) {
        console.warn(
          '[CepDashboard] Tenant Properties not found. ' +
          'Run deploy/set-tenant-properties.ps1 to set them.'
        );
      }
    } catch (e) {
      console.error('[CepDashboard] Error retrieving Tenant Properties:', e);
    }
  }
  ...
  private async _initApiClient(): Promise<void> {
    ...
    try {
      ...
      console.error('[CepDashboard] Failed to initialise AAD HTTP client:', e);
    }
  }
